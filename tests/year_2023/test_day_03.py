...
def test_find_adjacent_unique():
    ...
    numbers = np.array(
        [[0, 0, 1, 2, 2], [0, 2, 1, 0, 2]]
    )  # Remember these are the coordinates
...
def test_part1():
    ...
    single_adjacent_number = np.array(
        [
            [2],
            [0],
        ]
    ...
    number_sequences = get_number_sequences(
        grid, numbers, np.array([is_adjacent[1], is_adjacent[0]])
    )
