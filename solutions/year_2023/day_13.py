...
def parse_input(input_data: List[str]) -> List[np.array]:
...
def consequtive(arr: np.array, axis=0) -> np.array:
    if axis == 1:
        arr = arr.T
...
def find_reflection(arr: np.array, rows: np.array, axis=0, smudge=False) -> List[int]:
    if axis == 1:
        arr = arr.T
...
def part2(input_data: Optional[List[str]]) -> Union[str, int]:
    ...
    with Timer("Part 2"):
        mazes = parse_input(input_data)
        for maze in tqdm(mazes):
            rows = [_ for _ in range(maze.shape[0])]
            mirror_row = find_reflection(maze, rows, smudge=True)
            cols = [_ for _ in range(maze.shape[1])]
